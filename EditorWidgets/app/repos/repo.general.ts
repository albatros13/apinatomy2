...
import {ResourceName, Resource, ExternalResource, Type, MeasurableLocation, MaterialType, LyphType, CylindricalLyphType, BorderType, NodeType,
  CausalityType, MeasurableType, GroupType, ProcessType, OmegaTreeType,
  Publication, Correlation, ClinicalIndex, Coalescence
...
export class RepoGeneral extends RepoAbstract{
  ...
  ngOnInit(){
    ...
    if (this.types.length == 0)
      for (let x in ResourceName) this.types.push(x);
  ...
  protected onAdded(resourceType: any){
    ...
    if (clientLibrary){
      switch (resourceType){
        ...
        case this.resourceName.MeasurableLocation : newItem = model.MeasurableLocation.new({name: "New measurable location"}); break;
      ...
      switch (resourceType){
        case this.resourceName.ExternalResource : newItem = new ExternalResource({name: "New external resource"}); break;
        case this.resourceName.Type          : newItem = new Type({name: "New typed resource"}); break;
        case this.resourceName.MeasurableLocation : newItem = new MeasurableLocation({name: "New measurable location"}); break;
        ...
        case this.resourceName.Coalescence   : newItem = new Coalescence({name: "New coalescence"}); break;

        case this.resourceName.GroupType     : newItem = new GroupType({name: "New group"}); break;
        case this.resourceName.OmegaTreeType : newItem = new OmegaTreeType({name: "New omega tree", levels: []}); break;
