...
var treeRepo = new CanonicalTreeRepo([]);

myLayout.on('initialised', function() {
    var windowSize = {width: myLayout.width * 0.4 - 20, height: myLayout.height / 2 - 40};
    treeEditor(windowSize);
});


var treeEditor = function (windowSize) {

    //TreeViewer
    var svgTree = d3.select('.treeDefinition');
    var treeVP = new Plots.vp({
    ...
    //TreeSampleViewer
    var svgTreeSampleVP = new Plots.vp({
    ...
    lyphRepo.load("http://open-physiology.org:8889")
        .then(loadTreeRepo, logError);

    function loadTreeRepo(){
        return treeRepo.load("http://open-physiology.org:8889");
    }

    createTreeEditors();

    function createTreeEditors(){
